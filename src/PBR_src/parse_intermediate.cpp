...
/*
    std::string Token::ToString() const {
        return printf("[ Token token: %s loc: %s ]", toString(token), loc);
    }
    */
//    STAT_MEMORY_COUNTER("Memory/Tokenizer buffers", tokenizerMemory);

    // Tokenizer Implementation
    static char decodeEscaped(int ch, const PBR::FileLoc& loc) {
        switch (ch) {
        case EOF:
        ...
        case '\"':
            return '\"';
        default:
            printf("unexpected escaped character");
        }
        return 0;  // NOTREACHED
    }
...
/*
    std::unique_ptr<Tokenizer> Tokenizer::CreateFromFile(
        ...
        if (fd == -1) {
            errorCallback(printf("%s: %s", filename, ErrorString()).c_str(), nullptr);
            return nullptr;
        }
        ...
        auto errorReportLambda = [&errorCallback, &filename]() -> std::unique_ptr<Tokenizer> {
            ...
            errorCallback(printf("%s: %s", filename, messageBuffer).c_str(), nullptr);
...
*/
    ...
    std::unique_ptr<Tokenizer> Tokenizer::CreateFromString(
        std::string str, std::function<void(const char*, const PBR::FileLoc*)> errorCallback) {
        return std::make_unique<Tokenizer>(std::move(str), "<stdin>",
            std::move(errorCallback));
    }
    ...
    Tokenizer::Tokenizer(std::string str, std::string filename,
        ...
        loc = PBR::FileLoc(*new std::string(filename));
        pos = contents.data();
        end = pos + contents.size();
        tokenizerMemory += contents.size(); // mstack, is this ever user of defined anywhere?
    ...
    Tokenizer::~Tokenizer() {
        ...
        if (unmapPtr && unmapLength > 0)
            if (munmap(unmapPtr, unmapLength) != 0)
                errorCallback(printf("munmap: %s", ErrorString()).c_str(), nullptr);
#elif defined(PBRT_IS_WINDOWS)
        if (unmapPtr && UnmapViewOfFile(unmapPtr) == 0)
            errorCallback(printf("UnmapViewOfFile: %s", ErrorString()).c_str(),
                nullptr);
#endif
    }
...
//        std::vector<std::pair<AsyncJob<int>*, BasicSceneBuilder*>> imports; // Only sequential for now
...
//            std::string(t->loc.filename.begin(), t->loc.filename.end()));
        ...
        auto parseError = [&](const char* msg, const PBR::FileLoc* loc) {
 //           ErrorExit(loc, "%s", msg);
        };
                ...
                PBR::FileLoc loc) {
                    ...
                    ParsedParameterVector parameterVector = parseParameters(
                        nextToken, unget, formatting, [&](const Token& t, const char* msg) {
                            ...
                            //std::string str = printf("%s: %s", token, msg);
...
// need this!                    basicParamListEntrypoint(&ParserTarget::AreaLightSource, tok->loc);
            ...
            case 'C':
                if (tok->token == "ConcatTransform") {
                    ...
                    if (nextToken(TokenRequired)->token != "]")
                        syntaxError(*tok);
                    //target->ConcatTransform(m, tok->loc); // no
                }
                else if (tok->token == "CoordinateSystem") {
                    std::string_view n = dequoteString(*nextToken(TokenRequired));
                    //target->CoordinateSystem(toString(n), tok->loc); // no
                }
                else if (tok->token == "CoordSysTransform") {
                    std::string_view n = dequoteString(*nextToken(TokenRequired));
                    //target->CoordSysTransform(toString(n), tok->loc); // no
                }
                else if (tok->token == "ColorSpace") {
                    std::string_view n = dequoteString(*nextToken(TokenRequired));
                    //target->ColorSpace(toString(n), tok->loc); // no
                }
...
//                    target->MediumInterface(names[0], names[1], tok->loc);
            ...
            case 'N':
                if (tok->token == "NamedMaterial") { // need
                    std::string_view n = dequoteString(*nextToken(TokenRequired));
...
//                                "be replaced with AttributeBegin/End");
                ...
                else if (tok->token == "Transform") { // need
                    if (nextToken(TokenRequired)->token != "[")
                        syntaxError(*tok);
                    float m[16];
                    for (int i = 0; i < 16; ++i)
...
/*
    ...
    void ParseFiles(ParserTarget* target, std::vector<std::string> filenames) {
        auto tokError = [](const char* msg, const PBR::FileLoc* loc) {
            ErrorExit(loc, msg);
        };
/*
       ...
       // set up log
            ...
            for (const std::string& fn : filenames) {
             //   if (fn != "-")
             //       SetSearchDirectory(fn); // is this needed?

                std::unique_ptr<PBR::Tokenizer> t = Tokenizer::CreateFromFile(fn, tokError);
                if (t)
                    parse(target, std::move(t));
            }
      //  }
    ...
    /*
    // We really dont need these because we are assuming the non-FormattingParserTarget path of main..
    ...
    void FormattingParserTarget::Identity(PBR::FileLoc loc) {
        Printf("%sIdentity\n", indent());
    }

    void FormattingParserTarget::Translate(float dx, float dy, float dz, PBR::FileLoc loc) {
    ...
    void FormattingParserTarget::Scale(float sx, float sy, float sz, PBR::FileLoc loc) {
        Printf("%sScale %f %f %f\n", indent(), sx, sy, sz);
    }

    void FormattingParserTarget::LookAt(float ex, float ey, float ez, float lx, float ly,
        float lz, float ux, float uy, float uz, PBR::FileLoc loc) {
        Printf("%sLookAt %f %f %f\n%s    %f %f %f\n%s    %f %f %f\n", indent(), ex, ey, ez,
            indent(), lx, ly, lz, indent(), ux, uy, uz);
    }

    void FormattingParserTarget::ConcatTransform(float transform[16], PBR::FileLoc loc) {
    ...
    void FormattingParserTarget::Transform(float transform[16], PBR::FileLoc loc) {
    ...
    void FormattingParserTarget::CoordinateSystem(const std::string& name, PBR::FileLoc loc) {
        Printf("%sCoordinateSystem \"%s\"\n", indent(), name);
    }

    void FormattingParserTarget::CoordSysTransform(const std::string& name, PBR::FileLoc loc) {
        Printf("%sCoordSysTransform \"%s\"\n", indent(), name);
    }

    void FormattingParserTarget::ActiveTransformAll(PBR::FileLoc loc) {
        Printf("%sActiveTransform All\n", indent());
    }

    void FormattingParserTarget::ActiveTransformEndTime(PBR::FileLoc loc) {
        Printf("%sActiveTransform EndTime\n", indent());
    }

    void FormattingParserTarget::ActiveTransformStartTime(PBR::FileLoc loc) {
        Printf("%sActiveTransform StartTime\n", indent());
    }

    void FormattingParserTarget::TransformTimes(float start, float end, PBR::FileLoc loc) {
        Printf("%sTransformTimes %f %f\n", indent(), start, end);
    }

    void FormattingParserTarget::ColorSpace(const std::string& n, PBR::FileLoc loc) {
        Printf("%sColorSpace \"%s\"\n", indent(), n);
    }

    void FormattingParserTarget::PixelFilter(const std::string& name,
        ParsedParameterVector params, PBR::FileLoc loc) {
...
//                        "This value is now set via the Sampler's number of pixel samples.");
    ...
    void FormattingParserTarget::AttributeBegin(PBR::FileLoc loc) {
        Printf("\n%sAttributeBegin\n", indent());
        catIndentCount += 4;
    }

    void FormattingParserTarget::AttributeEnd(PBR::FileLoc loc) {
        catIndentCount -= 4;
    ...
    void FormattingParserTarget::TransformBegin(PBR::FileLoc loc) {
//        Warning(&loc, "Rewriting \"TransformBegin\" to \"AttributeBegin\".");
        Printf("%sAttributeBegin\n", indent());
        catIndentCount += 4;
    }

    void FormattingParserTarget::TransformEnd(PBR::FileLoc loc) {
        catIndentCount -= 4;
...
//                Warning(&loc, "%s: renaming multiply-defined texture", name);
                definedTextures[name] = printf("%s-renamed-%d", name, count++);
            }
            else
        ...
        if (upgrade) {
            ...
            if (texname == "imagemap" || texname == "ptex") {
                ...
                if (gamma != 0) {
                    ...
                    extra +=
                        indent(1) + printf("\"string encoding \" \"gamma %f\" ", gamma);
                }
                else {
    ...
    std::string FormattingParserTarget::upgradeMaterialIndex(const std::string& name,
        ...
        if (!tex.empty()) {
            ...
            return indent(1) + printf("\"texture eta\" \"%s\"\n", tex);
        }
        else {
            ...
            return indent(1) + printf("\"float eta\" [ %f ]\n", value);
        }
    }
            ...
            *name = "coateddiffuse";
            ...
            if (rgb) {
                if (rgb->r == rgb->g && rgb->g == rgb->b)
                    extra += indent(1) + printf("\"float amount\" [ %f ]\n", rgb->r);
                else {
            ...
            extra +=
                indent(1) + printf("\"string materials\" [ \"%s\" \"%s\" ]\n", m2, m1);
        }
    ...
    void FormattingParserTarget::MakeNamedMaterial(const std::string& name,
        ...
        if (upgrade) {
            ...
            if (definedNamedMaterials.find(name) != definedNamedMaterials.end()) {
                ...
                definedNamedMaterials[name] = printf("%s-renamed-%d", name, count++);
            }
            else
        ...
        if (upgrade) {
            ...
            extra = indent(1) + printf("\"string type\" [ \"%s\" ]\n", matName) + extra;
        }
        std::cout << extra << dict.ToParameterList(catIndentCount);
    ...
    void FormattingParserTarget::NamedMaterial(const std::string& name, PBR::FileLoc loc) {
    ...
    static std::string upgradeMapname(const FormattingParserTarget& scene,
        ...
        return scene.indent(1) + printf("\"string filename\" \"%s\"\n", n);
    }
    ...
    void FormattingParserTarget::Shape(const std::string& name, ParsedParameterVector params,
        ...
        if (toPly && name == "trianglemesh") {
            ...
            if (vi.size() < 500) {
                ...
                std::string fn = printf("%s_%05d.ply", plyPrefix, count++);
    ...
    void FormattingParserTarget::ReverseOrientation(PBR::FileLoc loc) {
        Printf("%sReverseOrientation\n", indent());
    }

    void FormattingParserTarget::ObjectBegin(const std::string& name, PBR::FileLoc loc) {
        if (upgrade) {
            if (definedObjectInstances.find(name) != definedObjectInstances.end()) {
                ...
                definedObjectInstances[name] = printf("%s-renamed-%d", name, count++);
            }
            else
    ...
    void FormattingParserTarget::ObjectEnd(PBR::FileLoc loc) {
        Printf("%sObjectEnd\n", indent());
    }

    void FormattingParserTarget::ObjectInstance(const std::string& name, PBR::FileLoc loc) {
        if (upgrade) {
            if (definedObjectInstances.find(name) == definedObjectInstances.end())
